...
export default function TabTwoScreen() {
    ...
    useEffect(() => {
        const getProducts = async () => {
            const res = await fetch('https://dummyjson.com/products?limit=200&skip=77');
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <View style={styles.card}>
                                    <TouchableWithoutFeedback onPressIn={() => { onPressCart(item.id) }}>
                                        <View style={styles.cartIcon}>
                                            <Ionicons name="cart" size={24} color={`${item.cartQuantity > 0 ? 'darkorange' : '#aaa'}`} />
                                        </View>
